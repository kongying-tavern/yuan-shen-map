...
L.TileLayer.T1 = L.TileLayer.extend({
  getTileUrl: function (coords) {
    ...
    y = coords.y
    if (x % 2 == 0)
      if (y % 2 == 0) return 'tiles2/SY.png'
      else return 'tiles2/SY1.png'
    else if (y % 2 == 0) return 'tiles2/SY1.png'
...
$('.leaflet-control-layers-overlays label')
  .eq(0)
  .before('<a class="ST-OPT assortType">神瞳</a>')
$('.leaflet-control-layers-overlays label')
  .eq(2)
  .before('<a class="DLK-OPT assortType">地灵龛</a>')
$('.leaflet-control-layers-overlays label')
  .eq(4)
  .before('<a class="KW-LY-OPT assortType">矿物——璃月</a>')
$('.leaflet-control-layers-overlays label')
  .eq(6)
  .before('<a class="KW-MD-OPT assortType">矿物——蒙德</a>')
$('.leaflet-control-layers-overlays label')
  .eq(8)
  .before('<a class="CJW-OPT assortType">采集物——璃月</a>')
$('.leaflet-control-layers-overlays label')
  .eq(13)
  .before('<a class="CJW-MD-OPT assortType">采集物——蒙德</a>')
$('.leaflet-control-layers-overlays label')
  .eq(20)
  .before('<a class="JYG-LY-OPT assortType">精英怪——璃月</a>')
...
for (let i = 0; i < $('.leaflet-control-layers-overlays label').length; i++) {
  if (
    $(`.leaflet-control-layers-overlays label:eq(${i}) div span span`).hasClass(
      'ST-OPT'
    ) == true
  ) {
    $(`.leaflet-control-layers-overlays label:eq(${i})`).addClass('ST-OPT')
    $(
      `.leaflet-control-layers-overlays label:eq(${i}) div span span`
    ).removeClass('ST-OPT')
  } else if (
    $(`.leaflet-control-layers-overlays label:eq(${i}) div span span`).hasClass(
      'DLK-OPT'
...
$('.leaflet-control-layers-overlays').append(
  '<div class="jquery-accordion-menu-footer"><a href="https://bbs.mihoyo.com/ys/accountCenter/postList?id=5284717">空荧酒馆@A8</a></div>'
)
$('.leaflet-control-layers-overlays').wrapInner(
  '<div class="htmleaf-container"/><form class="ac-custom ac-checkbox ac-checkmark" autocomplete="off"/><div id="jquery-accordion-menu" class="jquery-accordion-menu black"/>'
)
