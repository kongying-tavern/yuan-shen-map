...
function onEachFeature(feature, layer) {
  ...
  // popupHtml += '</div>';
  // popupHtml += '<div class="' + switchClass + '" onclick="MarkPoint(this)" data-key="' + key + '"><p class="switchOff"></p><p class="switchOn">終了しました</p><div class="switchButton"><div class="switchButtonIcon"><p></p></div></div></div>';
...
var typearray = [
  ...
  [LayerMap['Layer_SJRW_DQ'], JS_SJRW_DQ, 'TC', 'SJRW_DQ'],
  ...
  [LayerMap['Layer_JinGua_DQ'], JS_JinGua_DQ, 'SC', 'JinGua_DQ'],
  ...
  [LayerMap['Layer_ZJK_DQ'], JS_ZJK_DQ, 'TC', 'ZJK_DQ'],
  ...
  [LayerMap['Layer_SJRW_DQ2'], JS_SJRW_DQ2, 'TC', 'SJRW_DQ2'],
  ...
  [LayerMap['Layer_ZJK_DQ2'], JS_ZJK_DQ2, 'TC', 'ZJK_DQ2'],
...
function MarkPoint(element) {
  ...
  if (newValue) {
    ...
    setTimeout(function () {
      that.find('.switchButton p').html('完成した')
    }, 100)
  } else {
...
map.on('popupopen', function (e) {
  ...
  var switchText = !markedFlag ? '未完成' : '完成した'
  ...
  if (timeValue) {
    ...
    timer = setInterval(() => {
      ...
      $('.myPopContainer #time ').text(
        seconds >= 0
          ? `リフレッシュ時間：${day}日, ${hours}:${minutes}:${seconds}`
          : ''
      )
    }, 500)
  }
